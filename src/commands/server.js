  ...
  async execute(interaction) {
    if (interaction.options.getSubcommand() === "members") {
      await interaction.reply(
        `THERE ARE ${interaction.guild.memberCount} USERS IN THE SERVER!`
      );
    } else if (interaction.options.getSubcommand() === "createdat") {
      await interaction.reply(
        `THIS SERVER WAS BORN ON ${interaction.guild.createdAt}!`
      );
    } else if (interaction.options.getSubcommand() === "ownerid") {
      await interaction.reply(
        `THE USER ID OF THIS SERVER IS ${interaction.guild.ownerId}`
      );
    } else {
      await interaction.reply({
        content: "NO SUB COMMAND WAS USED!",
        ephemeral: true,
      });
    }
  },
